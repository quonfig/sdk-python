#!/usr/bin/env bash
#
# Run the cross-SDK chaos harness against sdk-python (qfg-47c2.8).
#
...
# 4. Runs `pytest chaos/test_chaos.py`.
# 5. Tears everything down on exit (success or failure).
#
# Mirrors sdk-go/scripts/run-chaos.sh and sdk-node/scripts/run-chaos.sh so
# every SDK has identical boot semantics.
#
...
export QUONFIG_CHAOS_SESSION="${QUONFIG_CHAOS_SESSION:-sdk-python-$$-$(date +%s)}"
...
CHAOS_API_DELIVERY_URL="http://127.0.0.1:$API_PORT" \
  mise exec -- poetry run pytest -v -s chaos/test_chaos.py "$@"
