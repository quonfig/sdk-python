...
def test_override_fires_when_quonfig_user_email_matches(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.dev-override', contexts={'quonfig-user': {'email': 'bob@foo.com'}})
...
def test_override_does_not_fire_when_attribute_absent_prod_simulation(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.dev-override', contexts={'user': {'email': 'bob@foo.com'}})
...
def test_override_matches_any_email_in_is_one_of_list(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.dev-override.multi-email', contexts={'quonfig-user': {'email': 'alice@foo.com'}})
    assert result is True
...
def test_override_beats_customer_rule_by_priority(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.dev-override.priority', contexts={'quonfig-user': {'email': 'bob@foo.com'}, 'user': {'country': 'DE'}})
    assert result is True
