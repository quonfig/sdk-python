...
def test_returns_true_from_global_context(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.in-seg.segment-and', contexts={'': {'domain': 'prefab.cloud'}, 'user': {'key': 'michael'}})
...
def test_returns_false_due_to_local_context_override(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.in-seg.segment-and', contexts={'': {'domain': 'prefab.cloud'}, 'user': {'key': 'james'}})
...
def test_returns_false_for_untouched_scope_context(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.in-seg.segment-and', contexts={'': {'domain': 'example.com'}, 'user': {'key': 'nobody'}})
    assert result is False
...
def test_returns_false_due_to_partial_scope_context_override_of_user_key(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.in-seg.segment-and', contexts={'': {'domain': 'example.com'}, 'user': {'key': 'michael'}})
    assert result is False
...
def test_returns_false_due_to_partial_scope_context_override_of_domain(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.in-seg.segment-and', contexts={'': {'domain': 'example.com', 'key': 'prefab.cloud'}, 'user': {'key': 'nobody'}})
    assert result is False


# returns true due to full scope context override of user.key and domain
def test_returns_true_due_to_full_scope_context_override_of_user_key_and_domain(config_client) -> None:
    c = config_client
    result = c.is_feature_enabled('feature-flag.in-seg.segment-and', contexts={'': {'domain': 'prefab.cloud'}, 'user': {'key': 'michael'}})
    assert result is True
