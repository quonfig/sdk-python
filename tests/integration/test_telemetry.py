...
def test_reason_is_split_for_weighted_value_evaluation() -> None:
    ...
    feed_aggregator(
        ...
        contexts={"user": {"tracking_id": "92a202f2"}},
    )
    assert aggregator_post(agg, "evaluation_summary", endpoint="/api/v1/telemetry") == [
        {
...
def test_evaluation_summary_groups_by_config_key() -> None:
    agg = build_aggregator("evaluation_summary", {})
    feed_aggregator(
        agg, "evaluation_summary", {"keys": ["brand.new.string", "always.true"]}, contexts={}
    )
    assert aggregator_post(agg, "evaluation_summary", endpoint="/api/v1/telemetry") == [
        {
            "key": "brand.new.string",
...
def test_context_shape_merges_fields_across_multiple_records() -> None:
    ...
    assert aggregator_post(agg, "context_shape", endpoint="/api/v1/context-shapes") == [
...
def test_example_contexts_deduplicates_by_key_value() -> None:
    ...
    feed_aggregator(
        ...
        [
            ...
            {"user": {"key": "user-123", "name": "bob"}},
        ],
        contexts={},
    )
    assert aggregator_post(agg, "example_contexts", endpoint="/api/v1/telemetry") == {
        "user": {"key": "user-123", "name": "alice"}
    }
...
def test_telemetry_disabled_emits_nothing() -> None:
    agg = build_aggregator(
        "evaluation_summary",
        {"collect_evaluation_summaries": False, "context_upload_mode": ":none"},
    )
    feed_aggregator(agg, "evaluation_summary", {"keys": ["brand.new.string"]}, contexts={})
...
def test_shapes_only_mode_reports_shapes_but_not_examples() -> None:
    ...
    feed_aggregator(
        agg, "context_shape", {"user": {"name": "alice", "key": "alice-123"}}, contexts={}
    )
    assert aggregator_post(agg, "context_shape", endpoint="/api/v1/context-shapes") == [
...
def test_log_level_evaluations_are_excluded_from_telemetry() -> None:
    agg = build_aggregator("evaluation_summary", {})
    feed_aggregator(
        agg, "evaluation_summary", {"keys": ["log-level.prefab.criteria_evaluator"]}, contexts={}
    )
    assert aggregator_post(agg, "evaluation_summary", endpoint="/api/v1/telemetry") is None
